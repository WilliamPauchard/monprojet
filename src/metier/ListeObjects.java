...
abstract class ListeObjects extends Observable {
    ...
    /* Constructeur */
    protected ListeObjects (Observer observer) {if (observer != null) {addObserver(observer);}}
    ...
    public final void setPos (int pos) {
        if (this.pos != pos) {
            this.pos = pos;
            setChanged(); notifyObservers(new Action(Action.SELECTION, pos));
        }
    } // setPos
    ...
    public final void delCourant () {
        ...
        liste.remove(pos);
        setChanged(); notifyObservers(new Action(Action.SUPPRESSION, pos));
        setPos(NO_POS);
    } // delCourant
    ...
    public final void del (int k) {
        ...
        liste.remove(k);
        setChanged(); notifyObservers(new Action(Action.SUPPRESSION, k));
        if (pos == k) {
            setPos(NO_POS);
        } else if (pos > k) {
            setPos(pos - 1);
    ...
    public final void add (Object obj, int k) {
        if (k < 0 || k > liste.size()) {return;}
        liste.add(k, obj);
        setChanged(); notifyObservers(new Action(Action.ADJONCTION, k));
        if (pos >= k) {setPos(pos + 1);}
    } // add
